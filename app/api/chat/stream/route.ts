...
- 통신판매업 신고, 영업신고 등 부가 절차를 포함하세요.`;
  ...
  // 보조금/정책자금
  if (/보조금|정책자금|지원금|정부지원|중소기업|벤처|스타트업|R&D|연구개발|수출바우처|이노비즈|메인비즈|경영혁신|기술혁신/.test(msg)) {
    return `\n\n[전문 분야: 보조금/정책자금]
...
🔴 핵심 규칙:
1. 사용자의 대화 맥락을 파악하여, 위 목록 중 **정확히 일치하는** 서식이 있을 때만 답변 마지막에 [[DOCUMENT:해당서식의code값]] 마커를 출력하세요.
2. **관련성 판단 기준**: 서식의 name과 category가 사용자 질문의 주제/분야와 **직접적으로** 관련될 때만 매칭하세요.
   - ❌ 금지: "이노비즈 사업계획서" 질문에 "4대보험 신고서" 매칭 (분야가 다름)
   - ❌ 금지: "보조금 안내" 질문에 "건축허가 신청서" 매칭 (분야가 다름)
   - ✅ 허용: "식당 영업신고" 질문에 "식품영업신고서" 매칭 (같은 분야)
3. **확실하지 않으면 마커를 출력하지 마세요.** 어설프게 다른 분야 서식을 보여주는 것보다, 마커 없이 정보만 안내하는 것이 훨씬 낫습니다.
4. 행정 절차 안내 시 **정확히 일치하는** 관련 서식이 있으면 "서류를 작성해드릴까요?" 라고 제안하고 마커를 출력하세요.
5. 마커 형식: [[DOCUMENT:정확한_code값]] (예: [[DOCUMENT:hwpx_식품영업신고서]])
6. 이 마커가 출력되면 사용자 화면에 서류 작성 폼이 자동으로 나타납니다.
7. 목록에 없는 서식은 절대 마커를 만들어내지 마세요.

예시:
사용자: "식당 열 거야" → ✅ [[DOCUMENT:hwpx_식품영업신고서]] (정확히 관련)
사용자: "이노비즈 인증 받고 싶어" → ❌ 4대보험 서식 매칭 금지 (분야 불일치). 이노비즈 관련 서식이 목록에 없으면 마커 없이 안내만 하세요.`;
