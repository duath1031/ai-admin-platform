...
export default function CertDiagnosis() {
  ...
  useEffect(() => {
    if (selectedClient) {
      // Use selectedClient data for diagnosis
      const companyData: CompanyData = {
        ...
        revenueYear3: selectedClient.revenueYear3 ? Number(selectedClient.revenueYear3) : null,
        rndExpenditure: selectedClient.rndExpenditure ? Number(selectedClient.rndExpenditure) : null,
        totalAssets: selectedClient.totalAssets ? Number(selectedClient.totalAssets) : null,
        totalLiabilities: selectedClient.totalLiabilities ? Number(selectedClient.totalLiabilities) : null,
        ...
        permanentEmployees: selectedClient.permanentEmployees ?? null,
        ceoGender: selectedClient.ceoGender ?? null,
      };
      setProfile(companyData);
      const diagnosticResults = runCertificationCheck(companyData);
    ...
    <div className="max-w-4xl mx-auto">
      ...
      {profile && (
        <div className="mb-4 px-4 py-2 bg-gray-50 rounded-lg text-sm text-gray-600">
          분석 대상: <span className="font-medium text-gray-900">
            {selectedClient ? selectedClient.companyName : String((profile as Record<string, unknown>).companyName || "미입력")}
          </span>
          {(() => {
            const biz = selectedClient?.bizType || String((profile as Record<string, unknown>).bizType || "");
            return biz ? <span className="ml-2">({biz})</span> : null;
          })()}
        </div>
      )}
